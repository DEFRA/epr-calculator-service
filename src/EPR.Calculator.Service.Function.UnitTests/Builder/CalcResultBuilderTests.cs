...
public class CalcResultBuilderTests : TestsFor<CalcResultBuilder>
{
    ...
    public async Task Build_ShouldReturnCalcResult_WithProjectedProducers_Billing()
    {
        ...
        mockCalcResultWriter.Verify(m => m.StoreProducerMaterialPackaging(It.IsAny<List<L1Producer>>(), CancellationToken.None), Times.Never);
    }
    ...
    [TestMethod]
    public async Task Build_ShouldReturnCalcResult_Read_Billing()
    {
        var runContext = TestDataHelper.BillingRun2026;
        var mockCalcResultProjectedProducersH1Data = new Mock<List<CalcResultH1ProjectedProducer>>();
        var mockCalcResultProjectedProducersH2Data = new Mock<List<CalcResultH2ProjectedProducer>>();
        ...
        mockModulationBuilder.Verify(m => m.ConstructAsync(It.IsAny<IReadOnlyDictionary<string, string>>(), It.IsAny<IImmutableList<MaterialDetail>>(), It.IsAny<CalcResultLaDisposalCostData>(), It.IsAny<SelfManagedConsumerWaste>()), Times.Never);
