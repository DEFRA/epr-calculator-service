...
public class CalcRunLaDisposalCostBuilderTests
{
    ...
    public async Task Should_Return_LA_Disposal_Costs()
    {
        ...
        // Act
        var lapcapDisposalCostResults = await builder.ConstructAsync(runContext, DummyData.Materials, calcResult.CalcResultLapcapData, calcResult.CalcResultLateReportingTonnageData, smcw);
    ...
    public void GetDecimalValue_InvalidDecimalString_ReturnsZero()
    {
        ...
        // Act
        var result = CalcRunLaDisposalCostBuilder.GetDecimalValue(value);

        // Assert
        Assert.AreEqual(0m, result);
    }

    private static void SeedDatabase(ApplicationDBContext context, CalculatorRunContext runContext)
    {
        var run = runContext.ToEntity();
        context.CalculatorRuns.Add(run);

        context.ProducerDetail.AddRange(
            new ProducerDetail { CalculatorRunId = run.Id, Id = 2 },
            new ProducerDetail { CalculatorRunId = run.Id, Id = 3 }
        );

        context.Material.AddRange(
            new() { Id = 1, Name = "Plastic", Code = "PL" },
            new() { Id = 2, Name = "Steel", Code = "ST" },
            new() { Id = 3, Name = "Glass", Code = "GL" }
            );
