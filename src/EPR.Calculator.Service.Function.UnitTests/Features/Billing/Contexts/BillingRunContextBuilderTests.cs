...
public class BillingRunContextBuilderTests
{
    ...
    public async Task Should_create_valid_context()
    {
        ...
        var runContext = await _sut.CreateContext(DbSeedData.Defaults.ValidForBillingRun.Id, "Test User", CancellationToken.None);
        runContext.RunId.ShouldBe(DbSeedData.Defaults.ValidForBillingRun.Id);
        runContext.RunName.ShouldBe(DbSeedData.Defaults.ValidForBillingRun.Name);
        runContext.RelativeYear.ShouldBe(DbSeedData.Defaults.ValidForBillingRun.RelativeYear);
    ...
    {
        var invalidRun = DbSeedData.Defaults.ValidForBillingRun;
        invalidRun.Id = 3;
        invalidRun.BillingRunStatus = BillingRunStatus.Completed;

        await _dbContext.CalculatorRuns.AddAsync(invalidRun);
        await _dbContext.SaveChangesAsync();

        var exception = await Should.ThrowAsync<RunContextException>(() =>
            _sut.CreateContext(3, "ignored", CancellationToken.None));

        exception.Message.ShouldContain("BillingRunStatus");
    }
    ...
    public async Task Should_throw_when_no_producers_accepted()
    {
        ...
        var exception = await Should.ThrowAsync<RunContextException>(() =>
            _sut.CreateContext(invalidRun.Id, "ignored", CancellationToken.None));
