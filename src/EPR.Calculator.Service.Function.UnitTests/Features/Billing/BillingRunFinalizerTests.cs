...
public class BillingRunFinalizerTests
{
    ...
    public async Task Should_commit_changes()
    {
        ...
        updated.SuggestedInvoiceAmount.ShouldBe(section.SuggestedInvoiceAmount);
    }
    ...
    {
        // Arrange
        var fees = _fixture.Create<List<CalcResultSummaryProducerDisposalFees>>();
        fees.ForEach(f => f.Level = CommonConstants.LevelTwo.ToString());

        _calcResult.CalcResultSummary.ProducerDisposalFees = fees;
    ...
    public async Task Should_commit_empty_values_when_level1_with_no_billing_instruction()
    {
        ...
        original.SuggestedBillingInstruction = BillingConstants.Suggestion.Initial;
