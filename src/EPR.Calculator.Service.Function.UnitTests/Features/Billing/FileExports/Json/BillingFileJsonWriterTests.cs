...
public class BillingFileJsonWriterTests
{
    ...
    private readonly BillingFileJsonWriter testClass;

    public BillingFileJsonWriterTests()
    {
        mockMaterialService = new Mock<IMaterialService>();
        testClass = new BillingFileJsonWriter(mockMaterialService.Object);
    }
    ...
    public void Export_ShouldReturnJsonContent()
    {
        ...
        mockMaterialService.Setup(service => service
                .GetMaterials(It.IsAny<CancellationToken>()))
            .Returns(Task.FromResult(TestDataHelper.Materials));

        // Act
        var result = testClass.WriteToString(runContext, calcResult);
