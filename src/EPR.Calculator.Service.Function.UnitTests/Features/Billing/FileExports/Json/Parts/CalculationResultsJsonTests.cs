...
public class CalculationResultsJsonFromTests
{
    ...
    {
        // Arrange
        var runContext = TestFixtures.Legacy.Create<BillingRunContext>() with { AcceptedProducerIds = [1, 2, 3] };
        var data = TestDataHelper.GetCalcResultSummary();

        // Act
        var obj = CalculationResultsJson.From(runContext, data, TestDataHelper.Materials);
