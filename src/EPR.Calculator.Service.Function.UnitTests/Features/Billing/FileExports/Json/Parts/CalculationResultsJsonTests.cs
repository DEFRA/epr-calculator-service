...
public class CalculationResultsJsonFromTests
{
    ...
    {
        // Arrange
        var runContext = TestFixtures.Default.Create<BillingRunContext>() with { AcceptedProducerIds = [1, 2, 3] };
        var data = TestData.GetCalcResultSummary();

        // Act
        var obj = CalculationResultsJson.From(runContext, data, TestData.Materials);
