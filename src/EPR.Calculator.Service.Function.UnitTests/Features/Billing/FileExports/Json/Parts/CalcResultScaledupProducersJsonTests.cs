...
public class CalcResultScaledupProducersJsonTests
{
    ...
    {
        var runContext = TestFixtures.Legacy.Create<BillingRunContext>() with { AcceptedProducerIds = [101001] };
        var scaled = DummyData.GetScaledupProducers();

        var result = CalcResultScaledupProducersJson.From(runContext, scaled, DummyData.Materials);
        ...
        var withRealAccepted = CalcResultScaledupProducersJson.From(acceptedReal, scaled, DummyData.Materials);
