...
public class TelemetryEventsTests
{
    ...
    {
        TelemetryEvents.Started(TestFixtures.Default.Create<CalculatorRunContext>()).Name.ShouldBe("PayCalRunStarted");
    }

    [TestMethod]
    public void Started_Should_include_run_context_properties()
    {
        var result = TelemetryEvents.Started(TestFixtures.Default.Create<CalculatorRunContext>());
    ...
    {
        TelemetryEvents.Completed(TestFixtures.Default.Create<CalculatorRunContext>(), TimeSpan.Zero).Name.ShouldBe("PayCalRunCompleted");
    }

    [TestMethod]
    public void Completed_Should_include_run_context_properties()
    {
        var result = TelemetryEvents.Completed(TestFixtures.Default.Create<CalculatorRunContext>(), TimeSpan.Zero);
        result.Properties["RunId"].ShouldBe("1");
        result.Properties["RunName"].ShouldStartWith("Test Run");
        result.Properties["RunType"].ShouldBe("Calculator");
    }
    ...
    public void Completed_Should_include_elapsed_ms()
    {
        ...
        TelemetryEvents.Completed(TestFixtures.Default.Create<CalculatorRunContext>(), elapsed).Properties["ElapsedMs"].ShouldBe("1500");
    }

    [TestMethod]
    public void Failed_Should_return_correct_event_name()
    {
        TelemetryEvents.Failed(TestFixtures.Default.Create<CalculatorRunContext>(), TimeSpan.Zero, "reason").Name.ShouldBe("PayCalRunFailed");
    }

    [TestMethod]
    public void Failed_Should_include_run_context_properties()
    {
        var result = TelemetryEvents.Failed(TestFixtures.Default.Create<CalculatorRunContext>(), TimeSpan.Zero, "reason");
        result.Properties["RunId"].ShouldBe("1");
        result.Properties["RunName"].ShouldStartWith("Test Run");
        result.Properties["RunType"].ShouldBe("Calculator");
    }
    ...
    public void Failed_Should_include_elapsed_ms()
    {
        ...
        TelemetryEvents.Failed(TestFixtures.Default.Create<CalculatorRunContext>(), elapsed, "reason").Properties["ElapsedMs"].ShouldBe("2000");
    }
    ...
    {
        TelemetryEvents.Failed(TestFixtures.Default.Create<CalculatorRunContext>(), TimeSpan.Zero, reason).Properties["FailureReason"].ShouldBe(reason);
    }
    ...
    {
        var result = TelemetryEvents.WarnException(new Exception(), TestFixtures.Default.Create<CalculatorRunContext>());
        result.Properties["RunId"].ShouldBe("1");
        result.Properties["RunName"].ShouldStartWith("Test Run");
        result.Properties["RunType"].ShouldBe("Calculator");
    }
    ...
    {
        var result = TelemetryEvents.ErrorException(new Exception(), TestFixtures.Default.Create<CalculatorRunContext>());
        result.Properties["RunId"].ShouldBe("1");
        result.Properties["RunName"].ShouldStartWith("Test Run");
        result.Properties["RunType"].ShouldBe("Calculator");
    }
    ...
    {
        var result = TelemetryEvents.CriticalException(new Exception(), TestFixtures.Default.Create<CalculatorRunContext>());
        result.Properties["RunId"].ShouldBe("1");
        result.Properties["RunName"].ShouldStartWith("Test Run");
        result.Properties["RunType"].ShouldBe("Calculator");
    }
