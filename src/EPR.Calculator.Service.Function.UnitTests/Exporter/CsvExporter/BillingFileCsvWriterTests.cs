...
public class BillingFileCsvWriterTests
{
    ...
    {
        // Arrange
        var results = TestFixtures.Legacy.Create<CalcResult>();
        var runContext = TestFixtures.Legacy.Create<BillingRunContext>() with { AcceptedProducerIds = results.CalcResultScaledupProducers.ScaledupProducers!.Select(t => t.ProducerId).Take(1).ToImmutableHashSet() };

        _lateReportingExporter.Setup(mock => mock
            .Export(It.IsAny<CalcResultLateReportingTonnage>()))
            .Returns(TestFixtures.Legacy.Create<string>());
