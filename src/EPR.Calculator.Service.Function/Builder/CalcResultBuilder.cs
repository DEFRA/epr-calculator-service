    ...
    : ICalcResultBuilder
{
    public async Task<CalcResult> BuildAsync(RunContext runContext)
    {
        ...
        if (result.CalcResultModulation is not null)
        {
            ...
            logger.LogDebug("calcResultProjectedProducerBuilder end...");

            if (runContext.RunType == RunType.Calculator)
            {
                logger.LogDebug("Storing projected producers started...");
                await projectedProducers.StoreProjectedProducers(
                    runContext.RunId,
                    calcResultProjectedProducers.H2ProjectedProducers?.ToList() ?? [],
                    calcResultProjectedProducers.H1ProjectedProducers?.ToList() ?? []
                );
                logger.LogDebug("Storing projected producers ended...");
            }
        }
        else
        {
            ...
            logger.LogDebug("calcResultScaledupProducersBuilder end...");
        }
