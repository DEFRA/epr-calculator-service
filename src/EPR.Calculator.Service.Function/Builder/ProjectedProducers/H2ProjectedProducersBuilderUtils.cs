...
namespace EPR.Calculator.Service.Function.Builder.ProjectedProducers
{
    public static class H2ProjectedProducersBuilderUtils
    {
        ...
        private static ImmutableDictionary<string, CalcResultH2ProjectedProducerMaterialTonnage> GetProjectedTonnages(IImmutableList<MaterialDetail> materials, List<ProducerReportedMaterial> reportedMaterials) =>
            materials.ToImmutableDictionary(m => m.Code, m => GetProjectedTonnage(m, reportedMaterials.Where(rm => rm.MaterialId == m.Id).ToList()));
            ...
            return new CalcResultH2ProjectedProducer
            {
                ...
                ProjectedTonnageByMaterial = producer.ProjectedTonnageByMaterial.ToImmutableDictionary(
                    ...
                    kvp => new CalcResultH2ProjectedProducerMaterialTonnage {
                        ...
                        HouseholdDrinksContainerTonnage             = kvp.Key == MaterialCodes.Glass ? prodGroup.Sum(p => p.ProjectedTonnageByMaterial[kvp.Key].HouseholdDrinksContainerTonnage ?? 0) : null,
                        ...
                        ProjectedHouseholdDrinksContainerTonnage    = kvp.Key == MaterialCodes.Glass ? prodGroup.Sum(p => p.ProjectedTonnageByMaterial[kvp.Key].ProjectedHouseholdDrinksContainerTonnage ?? 0) : null,
