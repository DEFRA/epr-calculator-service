...
public static class CalcResultSummaryUtil
{
        ...
        CalcResult calcResult)
    {
        ...
        var isParseSuccessful = decimal.TryParse(laDisposalCostDataDetail.DisposalCostPricePerTonne, NumberStyles.Currency, CultureInfo.GetCultureInfo("en-GB"), out var value);
        ...
        return (total, red: null, amber: null, green: null);
    }
    ...
    {
        return decimal.TryParse(calcResult.CalcResultParameterOtherCost.BadDebtProvision.Value.Replace("%", string.Empty), out var value)
        ...
        decimal? producerDisposalFeeTotal)
    {
        ...
        return decimal.TryParse(disposalCost.Replace("%", string.Empty), out var value)
    ...
    public static void SetHeaders(CalcResultSummary result, IEnumerable<MaterialDetail> materials, bool applyModulation)
    {
        ...
        var section1MaterialsIdx = 1 + startingHeaders().Count();
        var section1DisposalFeeIdx = section1MaterialsIdx + section1Materials(materials, applyModulation).Count();
        var section2aMaterialsIdx = section1DisposalFeeIdx + section1DisposalFee().Count();
        var section2aCommsIdx = section2aMaterialsIdx + section2aMaterials(materials).Count();
        var section1DisposalIdx = section2aCommsIdx + section1Disposal().Count();
        var section2aComms2aIdx = section1DisposalIdx + section2aComms().Count();
        var commsCost2aPercentageIdx = section2aComms2aIdx + commsCost2aPercentage().Count();
        var commsCost2bIdx = commsCost2aPercentageIdx + commsCost2b().Count();
        var commsCost2cIdx = commsCost2bIdx + commsCost2c().Count();
        var onePlus2A2B2CProducerIdx = commsCost2cIdx + commsCost2c().Count();
        var threeSaCostsSummaryIdx = onePlus2A2B2CProducerIdx + OnePlus2A2B2CProducer.GetHeaders().Count();
        var laDataPrepCostsProducerIdx = threeSaCostsSummaryIdx + ThreeSaCostsProducer.GetHeaders().Count();
        var saSetupCostsSummaryIdx = laDataPrepCostsProducerIdx + LaDataPrepCostsProducer.GetHeaders().Count();
        var totalBillBreakdownProducerIdx = saSetupCostsSummaryIdx + SaSetupCostsProducer.GetHeaders().Count();
        var billingInstructionsProducerIdx = totalBillBreakdownProducerIdx + TotalBillBreakdownProducer.GetHeaders().Count();
