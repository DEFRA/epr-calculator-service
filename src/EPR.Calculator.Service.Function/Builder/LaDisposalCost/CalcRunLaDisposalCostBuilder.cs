    ...
    : ICalcRunLaDisposalCostBuilder
{
    ...
    )
    {
        ...
        foreach (var detail in lapcapDetails)
        {
            ...
            var materialCode = materials.FirstOrDefault(x => x.Name == detail.Name)?.Code;

            laDisposalDetail.ActionedSelfManagedConsumerWasteTonnage =
                runContext.RequiresModulation
                    ? GetActionedSelfManagedConsumerWasteTonnageValue(smcw, laDisposalDetail, materialCode).ToString()
                    : string.Empty;

            laDisposalDetail.ProducerReportedTotalTonnage = GetProducerReportedTotalTonnage(laDisposalDetail, runContext.RequiresModulation).ToString();
        ...
        var header = GetHeader(runContext.RequiresModulation);
    ...
    private string GetReportedHouseholdDrinksContainerTonnage(string materialName)
    {
        ...
        var producerDataTotal = materialName == CommonConstants.Total
    ...
    {
        var isParseSuccessful = decimal.TryParse(value, CultureInfo.InvariantCulture, out var result);
    ...
    private async Task<List<ProducerData>> GetProducerData(RunContext runContext)
    {
        ...
        return await (
            ...
            join producerMaterial in dbContext.ProducerReportedMaterialProjected on producerDetail.Id equals producerMaterial.ProducerDetailId
