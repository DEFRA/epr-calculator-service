...
public interface IInvoicedProducerService
{
    ...
    public Task<ImmutableList<InvoicedProducerRecord>> GetInvoicedProducerRecords(RelativeYear relativeYear, ImmutableHashSet<int>? producerIdFilter = null, CancellationToken cancellationToken = default);
    public Task<ImmutableList<InvoicedProducerRecord>> GetLatestAcceptedInvoicedProducerRecords(RelativeYear relativeYear, CancellationToken cancellationToken = default);
}
    ...
    : IInvoicedProducerService
{
    ...
    public async Task<ImmutableHashSet<int>> GetInvoicedThenCancelledProducerIdsForYear(RelativeYear relativeYear, CancellationToken cancellationToken = default)
    {
        return await dbContext
            ...
            .Where(ctx =>
                ...
                && ctx.calculatorRun.RelativeYear == relativeYear
    ...
    public async Task<ImmutableList<InvoicedProducerRecord>> GetInvoicedProducerRecords(RelativeYear relativeYear, ImmutableHashSet<int>? producerIdFilter = null, CancellationToken cancellationToken = default)
    {
        if (producerIdFilter is not null)
        {
            if (producerIdFilter.Count == 0)
                return ImmutableList<InvoicedProducerRecord>.Empty;
            ...
            .Where(ctx =>
                ...
                && !dbContext.ProducerResultFileSuggestedBillingInstruction
                    .Any(b =>
                        ...
                        && b.CalculatorRun.RelativeYear == relativeYear)
            );
                ...
                dbContext.CalculatorRunOrganisationDataDetails
                    ...
                    .Join(
                        dbContext.CalculatorRunOrganisationDataDetails
                            ...
                            .Select(g => new
                            {
                                g.Key.OrganisationId,
                                g.Key.CalculatorRunOrganisationDataMasterId,
                                MaxDetailId = g.Max(d => d.Id)
                            }),
