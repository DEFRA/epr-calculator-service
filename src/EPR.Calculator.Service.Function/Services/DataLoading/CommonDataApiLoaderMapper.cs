        ...
        private static string? SafeParseRamRagRating(PomResponse pom, ILogger logger)
        {
            try
            {
                return string.IsNullOrWhiteSpace(pom.RamRagRating)
                    ? null
                    : RagRatingExtensions.ParseRag(pom.RamRagRating).ToDbValue();
            }
            catch (Exception ex)
            {
                logger.LogWarning(
                    ex,
                    "Invalid RAG rating OrganisationId: '{OrganisationId}' SubsidiaryId: '{SubsidiaryId}' SubmitterId: '{SubmitterId}' RamRagRating '{RamRagRating}'",
                    pom.OrganisationId,
                    pom.SubsidiaryId,
                    pom.SubmitterId,
                    pom.RamRagRating);
