    ...
    : ITransposePomAndOrgDataService
{
    public async Task Transpose(CalculatorRunContext runContext, CancellationToken cancellationToken)
    {
        ...
        // PERF: pre-build the (OrganisationName, SubsidiaryId) -> representative producer map.
        // Replaces O(orgs^2) `organisationDataDetails.First(...)` lookup that previously ran inside the loop.
        // Preserves the original semantics of choosing the first encountered record per (name, subsidiary).
        var producerByNameAndSub = new Dictionary<(string OrganisationName, string? SubsidiaryId), CalculatorRunOrganisationDataDetail>(organisationDataDetails.Count);

        foreach (var odd in organisationDataDetails)
        {
            var key = (odd.OrganisationName, odd.SubsidiaryId);
            producerByNameAndSub.TryAdd(key, odd);
        }
        ...
        foreach (var organisation in organisationDataDetails)
        {
            ...
            // Get the producer based on the latest submission period (O(1) via pre-built map).
            var producer = producerByNameAndSub[(organisation.OrganisationName, organisation.SubsidiaryId)];
