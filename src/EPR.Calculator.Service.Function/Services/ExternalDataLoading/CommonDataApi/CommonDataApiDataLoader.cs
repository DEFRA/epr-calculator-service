...
namespace EPR.Calculator.Service.Function.Services.ExternalDataLoading.CommonDataApi;

/// <inheritdoc />
[ExcludeFromCodeCoverage(Justification = "Not unit testable with in-memory database given its transactional nature")]
public class CommonDataApiDataLoader(
    IOptions<CommonDataApiLoaderOptions> options,
    IDbContextFactory<ApplicationDBContext> dbContextFactory,
    CommonDataApiHttpClient httpClient,
    TimeProvider timeProvider,
    ILogger<CommonDataApiDataLoader> logger)
    : IExternalDataLoader
{
    /// <inheritdoc />
    public async Task LoadOrgAndPomData(RunContext runContext, CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation(
            "CommonDataApiLoader: Finished. Poms:{TotalPoms}, Organisations:{TotalOrganisations}",
            totalPoms, totalOrgs);
    }
