...
{
    Task<SelfManagedConsumerWaste> Calculate(RunContext runContext, ImmutableList<MaterialDetail> materialDetails);
}
...
public class SelfManagedConsumerWasteService : ISelfManagedConsumerWasteService
{
    ...
    )
    {
        if (group.Count() == 1 && group.First().SubsidiaryId == null)
        {
            ...
            return new SingleL1(
                p.ProducerId,
                CalcResultSummaryUtil.GetReportedTonnage(projectedMaterialsLookup, p, material, RagRating.Red) +
                CalcResultSummaryUtil.GetReportedTonnage(projectedMaterialsLookup, p, material, RagRating.RedMedical),
                CalcResultSummaryUtil.GetReportedTonnage(projectedMaterialsLookup, p, material, RagRating.Amber) +
                CalcResultSummaryUtil.GetReportedTonnage(projectedMaterialsLookup, p, material, RagRating.AmberMedical),
                CalcResultSummaryUtil.GetReportedTonnage(projectedMaterialsLookup, p, material, RagRating.Green) +
                CalcResultSummaryUtil.GetReportedTonnage(projectedMaterialsLookup, p, material, RagRating.GreenMedical),
                CalcResultSummaryUtil.GetReportedTonnage(projectedMaterialsLookup, p, material),
                CalcResultSummaryUtil.GetTonnage(projectedMaterialsLookup, p, material, PackagingTypes.ConsumerWaste)
            );
        }
