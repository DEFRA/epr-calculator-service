...
{
    /// <summary>
    ///     Persists any required state changes to the database, then marks the billing run as
    ///     <see cref="BillingRunStatus.Completed" />.
    /// </summary>
    ...
    /// <summary>
    ///     Marks the billing run as <see cref="BillingRunStatus.Errored" />.
    /// </summary>
    ...
    : IBillingRunFinalizer
{
    public async Task FinalizeAsCompleted(BillingRunContext runContext, CalcResult calcResult, BillingFileExportResult exportResult, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await telemetry.TrackDuration("BillingRun.Finalizer.SaveSuggestedBillingFees", () =>
                SaveSuggestedBillingFees(runContext, calcResult, cancellationToken));

            await telemetry.TrackDuration("BillingRun.Finalizer.SaveExportMetadata", () =>
                SaveExportMetadata(exportResult, cancellationToken));

            await telemetry.TrackDuration("BillingRun.Finalizer.SaveRunCompleted", () =>
