...
public interface IBillingRunContextBuilder
{
    ...
    /// </exception>
    Task<BillingRunContext> CreateContext(int runId, string? user, CancellationToken cancellationToken);
}
...
{
    public async Task<BillingRunContext> CreateContext(int runId, string? user, CancellationToken cancellationToken)
    {
